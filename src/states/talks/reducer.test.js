...
describe('talkRecuders function', () => {
  it('should return the initial state when given by unknown action', () => {
    //arrange: untuk mengumpulkan seluruh kebutuhan yang diperlukan untuk menguji sebuah unit.
    const initialState = [];
    const action = { type: 'UNKNOWN' };

    //action: bagian aksi dari unit yang diuji. biasanya, ini hanya terdiri dari satu baris kode yaitu pemanggilan fungsi dari unit yang diuji.
    const nextState = talksReducer(initialState, action);

    //assert: untuk memeriksa efek atau hasil dari action yang telah dilakukan oleh unit yang diuji (nilai nextState harus sama dengan initialState).
